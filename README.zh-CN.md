...
现有的多模型协作工具各有其架构层面的局限：

**Oh My ClaudeCode (OMC)** 采用**星形架构** —— Claude 是中心枢纽，可以分别与 Codex 和 Gemini 通信，但 Codex 和 Gemini **互相完全不知道对方的存在**。当 Claude 转发给 Codex 时，Codex 完全不知道 Gemini 说了什么，反之亦然。每次 relay 是无状态的单次调用，没有共享历史。这意味着 Claude 每次都要手动总结并重新注入上下文，浪费 token 且丢失细节。

```
     OMC 星形架构                     Group Chat 网状架构

   Codex ← → Claude ← → Gemini     Codex ← → chat.md ← → Gemini
                ↑                          ↖      ↑      ↗
                |                            Claude
        (中心枢纽，单点)                (所有人读同一个文件)
   Codex ✗ Gemini (互不可见)        Codex ↔ Gemini (通过共享上下文)
```

**Claude Code Bridge (CCB)** 通过外部终端基础设施（tmux 会话、分屏终端、命名管道）打通模型间的通信。虽然可用，但部署代价很高 —— 需要 WSL + tmux + 桥接脚本 —— 且通信通道在编码环境之外。这是一个**系统级**集成方案，而你真正需要的是**应用级**的。

**Agent Group Chat** 采用完全不同的思路：不通过中心枢纽或外部终端路由消息，而是让所有参与者读写**同一个 Markdown 文件**（`chat.md`）。这形成了事实上的网状拓扑——每个模型都能看到每条消息，包括它从未直接通信过的模型发出的消息。整个系统作为轻量级 MCP 服务器在 Claude Code 内部运行——无需外部进程、无需终端基础设施、无需手动转发上下文。

| | OMC | CCB | **Group Chat** |
|---|:---:|:---:|:---:|
| 拓扑结构 | 星形（Claude 中心） | 桥接（终端管道） | **网状（共享文件）** |
| Codex 能看到 Gemini 的消息？ | 否 | 取决于配置 | **是，始终可见** |
| 持久化历史 | 否（无状态 relay） | 部分（终端滚动缓冲） | **是（chat.md）** |
| 外部依赖 | 无 | tmux + WSL + 脚本 | **无** |
| 部署复杂度 | 低 | 高 | **低** |
| 每次 relay 的 token 开销 | 高（重新注入上下文） | 中（管道转发） | **低（从磁盘读取）** |
